...
Both modes support multiple LLM providers (Google Gemini, OpenAI, Anthropic, Ollama, Custom) and use Gemini 2.5 Computer Use for visual browser automation.
...
- `sidepanel.tsx` - Main React chat interface, handles messaging and tool execution
...
- `settings.tsx` - Settings UI for LLM configuration
...
- `src/components/` - React components (LLMSettings, LanguageSwitch)
- `src/i18n.ts` - Internationalization setup (Chinese/English)
- `src/locales/` - Translation files (en/zh)
...
- `services/` - AI services (Gemini, Computer Use)

### Key Integrations

**AI SDK (Vercel)**: Uses `ai` package with `@ai-sdk/google` for Gemini integration.
